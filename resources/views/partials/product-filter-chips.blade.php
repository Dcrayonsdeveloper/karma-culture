...
@php
    ...
    // not also throw away "Red". fullUrlWithoutQuery() can only take the whole
    // parameter, which is right for the one-value filters below.
...
@if($kkHasAny)
    <div class="flex flex-wrap items-center gap-2 mb-5">
        ...
        @if($kkV['category'])
            <a href="{{ request()->fullUrlWithoutQuery('category') }}" class="{{ $kkChip }} hover:bg-[#6F9CA2]/10 transition-colors">
        ...
        @if($kkV['min_price'] !== null || $kkV['max_price'] !== null)
            ...
            @endphp
            <a href="{{ request()->fullUrlWithoutQuery(['min_price', 'max_price']) }}" class="{{ $kkChip }} hover:bg-[#6F9CA2]/10 transition-colors">
        ...
        @if($kkV['rating'] !== null)
            <a href="{{ request()->fullUrlWithoutQuery('rating') }}" class="{{ $kkChip }} hover:bg-[#6F9CA2]/10 transition-colors">
                {{ $kkV['rating'] }}+ Stars
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </a>
        @endif
        ...
        @if($kkV['in_stock'])
            <a href="{{ request()->fullUrlWithoutQuery('in_stock') }}" class="{{ $kkChip }} hover:bg-[#6F9CA2]/10 transition-colors">
                In Stock
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </a>
        @endif
        ...
        @if($kkV['on_sale'])
            <a href="{{ request()->fullUrlWithoutQuery('on_sale') }}" class="{{ $kkChip }} hover:bg-[#6F9CA2]/10 transition-colors">
                On Sale
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </a>
        @endif
