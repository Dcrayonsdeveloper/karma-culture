    ...
    <div x-show="open"
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b border-neutral-100 shrink-0">
            <a href="{{ url('/') }}" class="flex items-center">
                ...
                @else
                    <img src="{{ asset('images/update-logo.png') }}" alt="{{ config('app.name') }}" class="h-8 object-contain">
                @endif
            </a>
            <button @click="open = false" class="p-2.5 text-neutral-600 hover:text-neutral-600 rounded-full hover:bg-neutral-100 focus:outline-none focus:ring-2 focus:ring-[#3E2A1F]" aria-label="Close menu">
        ...
        <div class="px-4 py-3 border-b border-neutral-100 shrink-0">
            @guest
                <div class="flex gap-2">
                    <a href="{{ route('login') }}" class="flex-1 py-3 text-center text-sm font-semibold text-white bg-[#3E2A1F] hover:bg-[#5E3A26] rounded-lg transition-colors">Login</a>
            ...
            @else
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 bg-[#3E2A1F]/10 rounded-full flex items-center justify-center shrink-0">
                        @if(auth()->user()->avatar_url)
                            <img src="{{ auth()->user()->avatar_url }}" alt="{{ auth()->user()->full_name }}" class="w-full h-full rounded-full object-cover">
                        @else
                            <span class="text-sm font-semibold text-[#3E2A1F]">{{ substr(auth()->user()->first_name, 0, 1) }}</span>
                        @endif
                    </div>
                    <div class="min-w-0">
            ...
            <form action="{{ route('search') }}" method="GET">
                <div class="relative">
                    ...
                    <input type="text" name="q" placeholder="Search products..."
                           class="w-full pl-9 pr-3 py-2 text-sm bg-neutral-50 border border-neutral-200 rounded-lg focus:outline-none focus:border-[#3E2A1F] placeholder-neutral-400">
                </div>
            </form>
        </div>
        ...
        <nav class="flex-1 overflow-y-auto">
            <div class="py-2">
                <!-- Quick Links -->
                <a href="{{ url('/') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-neutral-700 hover:bg-neutral-50 {{ request()->routeIs('home') ? 'text-[#3E2A1F]! bg-[#3E2A1F]/5 font-medium' : '' }}">
                ...
                <div class="mt-2 pt-2 border-t border-neutral-100">
                            ...
                            <div x-data="{ expanded: false }">
                                ...
                                <div x-show="expanded" x-collapse>
                                    <div class="bg-neutral-50/50 py-1">
                                        <a href="{{ route('category.show', $cat) }}" class="block pl-8 pr-4 py-2 text-xs font-medium text-[#3E2A1F] hover:bg-neutral-100/50">
                                            View All {{ $cat->name }}
                                        </a>
                                        @foreach($cat->children as $child)
                                            <a href="{{ route('category.show', $child) }}" class="block pl-8 pr-4 py-2 text-sm text-neutral-600 hover:text-[#3E2A1F] hover:bg-neutral-100/50">
                                                {{ $child->name }}
                                            </a>
                                        @endforeach
                                    </div>
                    ...
                    <a href="{{ route('categories.index') }}" class="flex items-center gap-2 px-4 py-3 text-sm text-[#3E2A1F] hover:bg-[#3E2A1F]/5 font-medium">
                ...
                @auth
                    <div class="mt-2 pt-2 border-t border-neutral-100">
                        ...
                        @if(auth()->user()->deliveryPartner)
                            <a href="{{ route('delivery.login') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-[#3E2A1F] hover:bg-[#3E2A1F]/5 font-medium">
                                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @else
                            <a href="{{ route('account.become-delivery-partner') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-[#3E2A1F] hover:bg-[#3E2A1F]/5 font-medium">
                                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0"/>
                                </svg>
