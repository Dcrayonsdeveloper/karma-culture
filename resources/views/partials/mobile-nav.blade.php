...
<div x-data="{ open: false }"
    ...
    <div x-show="open"
        ...
        <div class="px-4 py-3 border-b border-neutral-100 shrink-0">
            @guest
                <div class="flex gap-2">
                    <a href="{{ route('login') }}" class="flex-1 py-3 text-center text-sm font-semibold text-white bg-[#2D1810] hover:bg-[#1f1109] rounded-lg transition-colors">Login</a>
        ...
        <nav class="flex-1 overflow-y-auto">
            <div class="py-2">
                <!-- Quick Links -->
                <a href="{{ url('/') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-neutral-700 hover:bg-neutral-50 {{ request()->routeIs('home') ? 'text-[#2D1810]! bg-[#2D1810]/5 font-medium' : '' }}">
                ...
                @auth
                    <div class="mt-2 pt-2 border-t border-neutral-100">
                        ...
                        {{-- Straight after the wishlist, because the header's own
                             pair is hidden below sm and this menu is the only way
                             to either list on a phone. --}}
                        <a href="{{ route('favourites') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-neutral-700 hover:bg-neutral-50">
                            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {{-- The storefront header's bell is hidden below sm, so on a phone
                             this menu is the only route to the notifications page - and it
                             listed every other account screen but that one. The unread count
                             is scoped with forCustomer() because admin alerts share this
                             table and are keyed only by user_id. --}}
                        @php
                            $unreadNotificationCount = auth()->user()->notifications()->forCustomer()->unread()->count();
                        @endphp
                        <a href="{{ route('account.notifications') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-neutral-700 hover:bg-neutral-50">
                            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
                            </svg>
                            Notifications
                            @if($unreadNotificationCount > 0)
                                <span class="ml-auto text-[10px] font-bold px-1.5 py-0.5 rounded-full leading-none bg-kk-tan-dark text-kk-cream">{{ $unreadNotificationCount }}</span>
                            @endif
                        </a>
                        ...
                        <a href="{{ route('account.notification-preferences') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-neutral-700 hover:bg-neutral-50">
                            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
