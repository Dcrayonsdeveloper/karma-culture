<x-layouts.admin>
    ...
    <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 1.25rem;">
        <a href="{{ route('admin.coupons.index') }}" class="btn-icon" style="flex-shrink: 0; color: #616161; text-decoration: none;">
    ...
    <form action="{{ route('admin.coupons.store') }}" method="POST">
        ...
        <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 1rem;">
            <div style="display: flex; flex-direction: column; gap: 1rem;">
                ...
                <div class="card" style="padding: 1.25rem;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 1rem;">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="2" maxlength="1000" class="form-textarea" placeholder="Optional description...">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="form-error">{{ $message }}</p>
                            @enderror
                        </div>
                                ...
                                <div style="padding: 1rem; background: #f6f6f7; border: 1px solid #e3e3e3; border-radius: 0.75rem; display: flex; flex-direction: column; gap: 1rem;">
                                    ...
                                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                                        <div>
                                            ...
                                            {{-- Required only while this block is on screen. A hard
                                                 `required` on a field hidden by x-show blocks submission
                                                 with a control the browser cannot even focus. --}}
                                            <input type="number" name="conditions[buy_qty]" id="conditions_buy_qty"
                                                   value="{{ old('conditions.buy_qty') }}" min="1" max="100" step="1"
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="conditions[get_qty]" id="conditions_get_qty"
                                                   value="{{ old('conditions.get_qty') }}" min="1" max="100" step="1"
                                                   inputmode="numeric" title="Enter a whole number between 1 and 100."
                                                   x-bind:required="couponType === 'buy_x_get_y'"
                    ...
                    <div style="display: flex; flex-direction: column; gap: 1rem;">
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                            <div>
                                ...
                                <input type="number" name="max_discount" id="max_discount" value="{{ old('max_discount') }}" step="0.01" min="0" max="9999999.99"
                            ...
                            <div>
                                ...
                                <input type="number" name="min_order_amount" id="min_order_amount" value="{{ old('min_order_amount') }}" step="0.01" min="0" max="9999999.99"
                                       inputmode="decimal" title="Enter an amount, up to two decimal places."
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                            <div>
                                ...
                                <input type="number" name="usage_limit" id="usage_limit" value="{{ old('usage_limit') }}" min="1" max="1000000" step="1"
                            ...
                            <div>
                                ...
                                <input type="number" name="usage_per_user" id="usage_per_user" value="{{ old('usage_per_user') }}" min="1" max="65535" step="1"
                ...
                {{-- Applicable Products --}}
                @php
                    // Resolve the names too, so a failed submit redraws real product
                    // chips instead of "Product #12".
                    $selectedProductIds = old('applicable_products', []) ?: [];
                ...
                @php
                    ...
                    $selectedCategories = array_map('intval', old('applicable_categories', []));
                @endphp
                    ...
                    <div style="display: flex; flex-direction: column; gap: 1rem;">
                        <div>
                            ...
                            <input type="datetime-local" name="starts_at" id="starts_at" value="{{ old('starts_at') }}"
                                   min="{{ $scheduleFloor }}" data-schedule-start class="form-input">
                            @error('starts_at')
                                <p class="form-error">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            ...
                            <input type="datetime-local" name="expires_at" id="expires_at" value="{{ old('expires_at') }}"
                                   min="{{ $scheduleFloor }}" data-schedule-end="starts_at" class="form-input">
                            @error('expires_at')
                                <p class="form-error">{{ $message }}</p>
                            @enderror
                        </div>
