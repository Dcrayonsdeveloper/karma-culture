            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
                ...
                <div class="xl:col-span-2 space-y-4">
                    ...
                    <div class="card p-5" x-data="kkSizes()">
                        ...
                        <p class="text-xs mb-4" style="color: #616161;">Every product needs at least one size. Each row is one size a customer can buy, picked from the sizes you keep under Products &rarr; Sizes. Leave Price or MRP blank and the row uses the product&rsquo;s. Measurements are optional and let the assistant advise on fit. Leave SKU blank and one is generated. Colours are set separately below.</p>
                                        ...
                                        <tr style="border-bottom:1px solid #f1f1f1;">
                                            <td style="padding:.4rem;">
                                                {{-- A shop that has not filled its Sizes list in yet gets the
                                                     ...
                                                     out of adding a product until they have visited another screen
                                                     would be a worse form than the one this replaces. The branch is
                                                     plain Blade because the list cannot change while the page is
                                                     open - only the rows can. --}}
                                                @if($kkSizeNames->isNotEmpty())
                                                    {{-- Same name, same aria-label, same posted value as the text box
                                                         it replaces: nothing downstream can tell the difference, and
                                                         the compare-at guard still finds this row by its aria-labels.
                                                         The options are drawn by Alpine so a row added after load gets
                                                         the list too, and optionsFor() carries whatever the row already
                                                         holds, so a value that is on no master list survives being
                                                         looked at. --}}
                    ...
                    <script>
                        ...
                        function kkSizes() {
                            return {
                                ...
                                // a value bounced back from a refused save, or one typed before
                                // the list existed - keeps it, at the top of that row's own
                                // dropdown, so simply opening the form cannot rewrite it.
