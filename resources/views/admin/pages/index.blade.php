<x-layouts.admin>
    ...
    {{-- No inline success banner. The admin layout already turns every session
         flash into a toast, so a saved or deleted page announced itself twice
         in identical words. The toast is the single flash channel here. --}}
        ...
        <div style="display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 1rem; border-bottom: 1px solid #e3e3e3;">
            <form action="{{ route('admin.pages.index') }}" method="GET" style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                ...
                <div style="position: relative; flex: 1; max-width: 24rem;">
                    ...
                    <input type="text" name="search" maxlength="100" aria-label="Search" value="{{ request('search') }}" placeholder="Search pages..."
                        ...
                        <tr style="cursor: pointer; border-bottom: 1px solid #e3e3e3;"
                            ...
                            <td style="padding: 0.5rem 1rem; text-align: right;" onclick="event.stopPropagation();">
                                <div style="display: inline-flex; align-items: center; gap: 0.25rem;">
                                    ...
                                    @endif
                                    <form action="{{ route('admin.pages.toggle-status', $page) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" title="{{ $page->is_published ? 'Move to drafts' : 'Publish' }}"
                                                style="display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 0.375rem; color: #616161; background: transparent; border: none; cursor: pointer;"
                                                onmouseover="this.style.backgroundColor='#e3e3e3'" onmouseout="this.style.backgroundColor='transparent'">
                                            @if($page->is_published)
                                                {{-- Crossed-out eye: take the page down --}}
                                                <svg style="width: 15px; height: 15px;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                                            @else
                                                {{-- Upward arrow: put the page live --}}
