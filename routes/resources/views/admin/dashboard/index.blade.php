<x-layouts.admin>
    ...
    <div style="margin-bottom: 1.25rem;" x-data>
        <form method="GET" action="{{ route('admin.dashboard') }}" x-ref="filterForm" style="display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap;">
            ...
            @if($hasDateFilter)
                <a href="{{ route('admin.dashboard') }}" style="padding: 0.375rem 0.75rem; font-size: 12px; font-weight: 500; border-radius: 50rem; color: #3E2A1F; text-decoration: none;">Clear filter</a>
            @endif
        </form>
        @if($hasDateFilter)
