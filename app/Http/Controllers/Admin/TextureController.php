...
/**
 * The Textures list - the textures the product form OFFERS, not the textures
 * the shop has.
 *
 ...
 * row is a picker entry: a word that appears in the list when somebody builds
 * a product. The product then stores its own copy - into products.attributes
 * -> "Textures" - and the cart and order lines store the word again. Nothing
 * holds this row's id. So:
 *
 *   - renaming a row renames the entry on offer and nothing else; every
 *     product already carrying "Velvet" still says Velvet;
 *   - deleting a row takes the word off the picker and leaves every product,
 ...
 *
 * There is no hex here, unlike Colours: a texture is a surface rather than a
 * shade, so the only thing that can stand in for it visually is a photograph -
 * hence image_url and nothing else.
 */
class TextureController extends Controller
{
     ...
     *
     * 60 characters is not a taste decision: cart_items.texture and
     * order_items.texture are varchar(60). A longer name could be saved onto a
     ...
     */
    private function rules(?Texture $texture = null): array
    {
        return [
            'name' => [...V::text(max: 60, min: 1), $this->uniqueKey($texture)],
    ...
    {
        return V::imageMessages('image', 2048);
    }
     ...
     * `key` with - trims, collapses whitespace and lower-cases, so "Matte",
     * "matte" and "Matte " are one identity. That is deliberately the same
     * grouping the shop's filter rails use, and it is why a plain
     * unique:textures,name would not do: two rows differing only in case pass a
     ...
     * see and correct.
     */
    private function uniqueKey(?Texture $texture): Closure
    {
        return function (string $attribute, mixed $value, Closure $fail) use ($texture): void {
            $rule = Rule::unique('textures', 'key')->ignore($texture?->id);
            $key = ShopFilterCatalogue::normaliseKey((string) $value);

            if (Validator::make(['key' => $key], ['key' => [$rule]])->fails()) {
                $fail('That texture is already on the list. Names are matched without case or spacing, so "Matte" and "matte" would be the same entry.');
            }
        };
    }
    ...
    public function index(): View
    {
        ...
        $rows = Texture::ordered()->paginate($perPage)->withQueryString();
        ...
        // products spell their textures out rather than pointing at these rows.
        ...
        // includeHidden, because a texture an admin has hidden from the shop
        // rail is still a texture products are carrying and this screen has to
        // say so.
        $usage = collect(ShopFilterCatalogue::values('texture', includeHidden: true))
            ->mapWithKeys(fn ($v) => [$v->key => $v->count])
            ->all();

        return view('admin.textures.index', compact('rows', 'usage'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($request->hasFile('image')) {
            $data['image_url'] = $request->file('image')->store('textures', 'public');
        }
        ...
        // texture an admin just typed jumping to the top of the picker.
        $data['position'] = (Texture::max('position') ?? 0) + 1;

        Texture::create($data);

        return redirect()->route('admin.textures.index')->with('success', 'Texture added to the picker.');
    }

    public function edit(Texture $texture): View
    {
        return view('admin.textures.edit', ['row' => $texture]);
    }

    public function update(Request $request, Texture $texture): RedirectResponse
    {
        ...
        // sends the row back to its text-only entry. Either way the orphan is
        ...
        // dozen times leaving a dozen dead files under storage/textures.
        if ($request->hasFile('image')) {
            if ($texture->image_url) {
                Storage::disk('public')->delete($texture->image_url);
            }
            $data['image_url'] = $request->file('image')->store('textures', 'public');
        } elseif ($request->boolean('remove_image') && $texture->image_url) {
            Storage::disk('public')->delete($texture->image_url);
            $data['image_url'] = null;
        }
        ...
        return redirect()->route('admin.textures.index')
            ->with('success', 'Texture updated. Products already using the old spelling keep it - they store their own copy.');
    }
     ...
     * product's textures live in its own attributes JSON, and
     * cart_items.texture and order_items.texture are plain varchar columns
     * holding the word itself. Removing the row takes the texture off the
     * picker; the storefront, every open cart and every historical order are
     * untouched.
     *
     * The uploaded swatch image IS this row's own file, though, so it goes with
     * it rather than being orphaned on the public disk.
     */
    public function destroy(Texture $texture): RedirectResponse
    {
        if ($texture->image_url) {
            Storage::disk('public')->delete($texture->image_url);
        }
        ...
        return redirect()->route('admin.textures.index')
            ->with('success', 'Texture removed from the picker. Products, carts and orders already using it are unchanged.');
    }

    /**
     * Offer this texture on the product form, or stop offering it.
     *
     * Hiding is the gentler alternative to deleting for a texture being
     * retired: it disappears from new products without disturbing the ones that
     * have it.
     */
    public function toggle(Texture $texture): RedirectResponse
    {
        $texture->update(['is_active' => ! $texture->is_active]);

        return back()->with('success', $texture->is_active
            ? '"'.$texture->name.'" is offered on the product form again.'
            : '"'.$texture->name.'" is hidden from the product form. Products already using it are unchanged.');
    }

    public function move(Texture $texture, string $direction): RedirectResponse
    {
        abort_unless(in_array($direction, ['up', 'down'], true), 404);

        $this->swapPosition($texture, $direction);

        return back()->with('success', 'Texture order updated.');
    }
     ...
     */
    private function swapPosition(Texture $row, string $direction): void
    {
        $up = $direction === 'up';

        $neighbour = Texture::query()
            ->when(
                $up,
