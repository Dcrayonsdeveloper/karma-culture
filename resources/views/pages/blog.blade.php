                    ...
                    <article class="bg-white rounded-xl border border-neutral-100 overflow-hidden hover:shadow-md transition-all duration-200 group flex flex-col">
                        ...
                        <a href="{{ route('blog.show', $post->slug) }}" class="block shrink-0">
                            @if($post->featured_image)
                                {{-- Covers are uploaded editorially at whatever ratio the author
                                     had, so the card shows the whole picture over a blurred copy
                                     of it instead of cropping it down to 16:9. --}}
                                <x-media :src="asset_v('storage/' . $post->featured_image)"
                                         :alt="$post->title"
                                         zoom
                                         class="aspect-video overflow-hidden" />
                            @else
                                <div class="aspect-video flex items-center justify-center bg-neutral-50">
