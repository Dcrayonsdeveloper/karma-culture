<x-layouts.app>
        ...
        <div class="kk-pdp">
            ...
            <div class="kk-pdp__info" x-ref="buyBox">
                ...
                @else
                    ...
                    {{-- Just the status and a way onwards. The email capture that used
                         to sit here asked the shopper for something before it had given
                         them anything, on the one page where the thing they came for
                         isn't available. --}}
                    <div class="kk-pdp__oos">
                        <div class="kk-pdp__oos-head">
                            ...
                            <div>
                                ...
                                <p class="kk-pdp__oos-sub">This piece is out of stock right now. Do check back soon.</p>
                            </div>
                        </div>
            ...
            <div style="display:flex;flex-direction:column;gap:0;">
                ...
                <div style="border:1px solid #e5e5e5;border-radius:0.5rem;overflow:hidden;{{ $qi > 0 ? 'margin-top:-1px;' : '' }}">
                    <button @click="openFaq = openFaq === {{ $qi }} ? null : {{ $qi }}"
                        ...
                        {{-- Object form: a string :style replaces the whole attribute, which took
                             the width/height with it and left the chevron at its intrinsic size. --}}
                        <svg style="width:1.25rem;height:1.25rem;color:#565959;flex-shrink:0;transition:transform 0.2s;"
                             :style="{ transform: openFaq === {{ $qi }} ? 'rotate(180deg)' : '' }"
    ...
    function productPage() {
        return {
            ...
            // viewer, which is the only zoom that makes sense with a finger.
            zoomAvailable() {
            ...
            zoomImage() {
                ...
                const img = slide.querySelector('img:not(.kk-media__fill)');
            ...
            zoomDraw() {
                ...
                if (!frame || !pt || !img || this.showZoom) { this.zoomLeave(); return; }

                const r = frame.getBoundingClientRect();
                if (!r.width || !r.height) { this.zoomLeave(); return; }

                const x = pt.x - r.left, y = pt.y - r.top;
                // The pointer can end up off the photo without a mouseleave -
                // the gallery is sticky, so the frame slides out from under a
