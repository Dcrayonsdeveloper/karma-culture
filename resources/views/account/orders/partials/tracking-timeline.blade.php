{{-- The order's progress through Ordered → Packed → Shipped → Out for Delivery
     → Delivered. Rendered both on the dedicated Track page and directly on the
     order detail page, so a customer can see where their parcel is without
     having to click through to a second page.

     $order          the order being shown
     $trackingSteps  $order->getTrackingSteps()
     $showEta        draw the estimated-delivery strip; pass false where the
                     page already prints the expected date elsewhere --}}
@php
    $showEta = $showEta ?? true;
@endphp

<div class="bg-white border border-neutral-100 rounded-xl mb-6 overflow-hidden">
    {{-- Expected Delivery Strip --}}
    @if($showEta && $order->expected_delivery_date && !$order->delivered_at && !in_array($order->status, ['cancelled', 'returned']))
