<x-layouts.app>
    ...
    {{-- The address applications go to was written into the page, so it stayed
         careers@ after the store's own contact address changed. There is no
         separate careers mailbox to configure, so this is the one address the
         admin maintains under Online Store > Site Settings. --}}
    @php
        $kkCareersEmail = trim((string) \App\Models\Setting::get('contact_email', ''));
    @endphp
            ...
            <div class="max-w-3xl mx-auto text-center">
                ...
                <div class="bg-white rounded-xl border border-neutral-100 p-8 sm:p-12">
                    ...
                    <div class="inline-flex items-center gap-2 max-w-full min-w-0 bg-neutral-50 rounded-lg px-5 py-3 border border-neutral-100">
                        ...
                        <a href="mailto:{{ $kkCareersEmail }}" class="min-w-0 [overflow-wrap:anywhere] text-sm font-medium text-[#6F9CA2] hover:underline">{{ $kkCareersEmail }}</a>
                    </div>
                    @endif
                </div>
