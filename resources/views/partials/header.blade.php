    ...
    <div class="w-full px-3 lg:px-4">
        ...
        <div class="relative flex items-center justify-between h-16 lg:h-20">
            ...
            <div class="flex items-center gap-1 lg:gap-0 flex-1 justify-end">
                ...
                {{-- Favourites. Immediately beside the wishlist, in the wishlist's
                     own padding rather than the tighter p-2 lg:p-1.5 the rest of
                     the cluster uses, so the two save lists read as a pair and
                     their badges sit on the same line. Same hidden sm:flex too:
                     below sm the whole cluster gives way to the mobile nav, which
                     carries its own Favourites entry. --}}
                <a href="{{ route('favourites') }}" class="relative p-2.5 lg:p-2 text-kk-brown hover:text-kk-tan-dark transition-colors hidden sm:flex" aria-label="Favourites">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.562.562 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z"/>
                    </svg>
                    <span x-cloak
                          x-show="$store.favourites.count > 0"
                          x-text="$store.favourites.count"
                          class="absolute -top-0.5 -right-0.5 w-4 h-4 bg-kk-tan-dark text-kk-cream text-[10px] font-bold rounded-full flex items-center justify-center">
                    </span>
                </a>
