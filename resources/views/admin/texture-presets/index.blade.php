<x-layouts.admin>
    ...
    <div class="card" style="overflow: hidden;">
        <div style="padding: 0.75rem 1rem; border-bottom: 1px solid #e3e3e3;">
            <p style="font-size: 13px; color: #616161; margin: 0;">Reusable textures you can tick on a product. Give one a swatch image and the home page rail hangs that fabric on the shirt instead of a plain colour.</p>
        </div>
                ...
                <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                    <thead>
                        <tr style="border-bottom: 1px solid #e3e3e3;">
                            ...
                            <th style="padding: 0.5rem 1rem; text-align: left; font-weight: 500; color: #616161; font-size: 12px;">Swatch</th>
                        ...
                        @foreach($presets as $preset)
                            <tr onclick="window.location='{{ route('admin.texture-presets.edit', $preset) }}'" style="cursor: pointer; border-bottom: 1px solid #e3e3e3;" onmouseover="this.style.backgroundColor='#f6f6f7'" onmouseout="this.style.backgroundColor='transparent'">
                                ...
                                <td style="padding: 0.5rem 1rem;">
                                    @if($preset->image_src)
                                        <img src="{{ $preset->image_src }}" alt="" loading="lazy"
                                             style="width: 2.25rem; height: 2.25rem; object-fit: cover; border-radius: 0.25rem; border: 1px solid #e3e3e3; display: block;">
                                    @else
                                        <span style="color: #9e9e9e; font-size: 12px;">&mdash;</span>
                                    @endif
                                </td>
                                <td style="padding: 0.5rem 1rem;">
                                    @if($preset->is_active)
